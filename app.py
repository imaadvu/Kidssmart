# app.py — KidsSmart+ (raw HTML to extractor + proper title fallback + modal/inline auth + persisted results)
...
# Put your real key here OR use Streamlit Secrets:
...
def open_auth_modal():
    ...
    def _auth():
        st.write("Create an account or sign in to view full details.")
        tab_login, tab_register = st.tabs(["Login", "Register"])
                ...
                for i, r in enumerate(results, start=1):
                    ...
                    for row in rows:
                        ...
                        if not row.get("mode") or row["mode"] == "Unknown":
                            row["mode"] = "Online" if "online" in combined.lower() else row["mode"]
